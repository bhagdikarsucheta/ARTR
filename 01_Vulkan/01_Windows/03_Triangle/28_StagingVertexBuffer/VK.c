...
VkResult createVertexBuffer(void)
{
	...
	vkBufferCreateInfo_stagingBuffer.flags = 0;				//valid flags are used in scattered buffer(sparse buffer)
	vkBufferCreateInfo_stagingBuffer.size = sizeof(traingle_position);
	vkBufferCreateInfo_stagingBuffer.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;  //Tansfer Data as a Source, copy data from staging buffer so this is Src Buffer
	vkBufferCreateInfo_stagingBuffer.sharingMode = VK_SHARING_MODE_EXCLUSIVE;		//Use this buffer for Multi-threading

	vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo_stagingBuffer, NULL, &vertexData_stagingBuffer_position.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkCreateBuffer() failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkAllocateMemory failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkBindBufferMemory failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkMapMemory failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkUnmapMemory failed for  Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	//No need for vkMapMemory() memcpy() vkUnmapMmemory() for this visible device buffer
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkAllocateCommandBuffers failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkBeginCommandBuffer() failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\ncreateVertexBuffer()->vkEndCommandBuffer() failed for Staging Buffer Function is %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	//Now submit above command buffer to the queue
	VkSubmitInfo vkSubmitInfo;
	memset((void*)&vkSubmitInfo, 0, sizeof(VkSubmitInfo));
	...
	//No need of dstmask because no need of synchronization
	vkResult = vkQueueSubmit(vkQueue, 1, &vkSubmitInfo, VK_NULL_HANDLE);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\nvkQueueSubmit()->createVertexBuffer() Staging Buffer Function is Failed %d\n", vkResult);
		return vkResult;
	}
	else
	{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "\nvkQueueWaitIdle()->createVertexBuffer() Staging Buffer Function is Failed %d\n", vkResult);
		return vkResult;
	}
	else
	{
