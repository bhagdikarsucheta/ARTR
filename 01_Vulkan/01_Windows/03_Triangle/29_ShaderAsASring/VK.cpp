...
#include<SPIRV/GlslangToSpv.h>   //started needing recently
...
//gllang/spirv related libraries
...
VkResult createShaders(void)
{
	...
	if (bDone == FALSE)
	{
		...
		fprintf(gpFile, "\ncreateShaders()->glsl2Spv() failed for VERTEX Function is %d\n", vkResult);
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	{
		fprintf(gpFile, "\ncreateShaders()->vkCreateShaderModule VERTEX Function is Succeded");
	}
	...
	if (bDone == FALSE)
	{
		...
		fprintf(gpFile, "\ncreateShaders()->glsl2Spv() failed for FRAGMENT Function is %d\n", vkResult);
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
	}
	...
	{
		fprintf(gpFile, "\ncreateShaders()->vkCreateShaderModule FRAGMENT Function for fragment is Succeded");
	}
...
bool glsl2Spv(const VkShaderStageFlagBits shader_type, const char* pShader, vector<unsigned int>& Spv)
{
	...
	shaderStrings[0] = pShader;				///Initialize built in shader resources

	TBuiltInResource resources;
	initBuiltInResources(resources);

	//Set our shader string in above array as TShaderString's
	...
	//Parse the TShader to Compile : Means to check string of shader from beggining to End
	if (!shader.parse(&resources, 100, false, ruleMessages))
	{
		fprintf(gpFile, "\nglsl2Spv()->parse failed Function with Log %s\n", shader.getInfoLog());
		fprintf(gpFile, "\nglsl2Spv()->parse failed Function with DebugLog %s\n", shader.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gpFile, "\nglsl2Spv()->parse Succeded Function with Log %s\n", shader.getInfoLog());

	}

	//Add Shader to TProgram
	glslang::TProgram program;
	program.addShader(&shader);					//No Parametrized constructor

	//Link the Progam
	if (!program.link(ruleMessages))
	{
		fprintf(gpFile, "\nglsl2Spv()->link failed Function with Log %s\n", shader.getInfoLog());
		fprintf(gpFile, "\nglsl2Spv()->link failed Function with DebugLog %s\n", shader.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gpFile, "\nglsl2Spv()->link Succeded Function with Log %s\n", shader.getInfoLog());

	}


	////////AND NOW SEND THIS WHOLE COOKED PROGRAN AND SPIRV CODE THING TO GLSLANG LIBRARY
